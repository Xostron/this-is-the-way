...
export default function Alarm({ buildId }) {
		...
		<>
			{(!!alr?.alrClosed?.val || !!alr?.tout?.val || !!alr?.hout?.val || !!alr?.antibliz?.val || !!alr?.timer?.val) && (
				<div className='line-alarm'>
					{!!alr.alrClosed.val && (
						<img className={`icon `} src={def['alrClosed']} title={alr.alrClosed.title} />
					)}
					{!!alr.tout.val && <img className={`icon `} src={def['tout']} title={alr.tout.title} />}
					{!!alr.hout.val && <img className={`icon `} src={def['hout']} title={alr.hout.title} />}
					{!!alr.antibliz.val && (
						<img className={`icon `} src={def['antibliz']} title={alr.antibliz.title} />
					)}
					{!!alr.timer.val && (
						<div className='timer'>
							<img className={`icon `} src={def['timer']} title={alr.timer.title} />
							<span className={`count`}>{alr?.timer?.val}</span>
						</div>
					)}
				</div>
			)}
